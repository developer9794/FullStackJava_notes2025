...
**Collection** : In the java.util.package, there is an interface called a collection. It\'s used to represent a collection of separate objects as a single entity.

The collection root interface of all the collection . It has a number of classes and interfaces for representing a collection of individual objects as a single unit.

The sub-interfaces of the collection interface are List, Set, and Queue.Although the map interface is part of the Java collection framework, it does not inherit the interface\'s collection. The Collection interface\'s most significant functions are add(), remove(), clear(),
size(), and contains().
